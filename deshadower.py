...
class Deshadower(object):
    ...
    def setup_model(self):
        ...
        with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope()):
            ...
            # build the model
            self.shadow_free_image, self.predicted_mask=build_aggasatt_joint(self.input, self.channel, vgg_19_path=self.vgg_19_path)

            loss_mask = tf.reduce_mean(input_tensor=tf.keras.losses.binary_crossentropy(gtmask,tf.nn.sigmoid(self.predicted_mask)))
